...
\challengeheading{participation-disruption}{Variable Client Participation Disrupts Search}
...
\solutionheading{timing-based-groups}{Group split-model segments by training time} Split-model strategies can measure client computation and communication times and group segments that finish at similar times. Unlike \solutionref{resource-data-groups}, this groups clients only by observed timing so that supernet training waits less for the slowest segment~\cite{nasfly_2024}.

\solutionheading{staleness-weighting}{Adjust the influence of outdated search updates} The server can account for delay when adding a late update after the global search has advanced~\cite{rl_fednas_2021}. Differentiable search can instead give older updates less weight~\cite{fedregnas_2025}. Unlike \solutionref{exposure-aware-aggregation}, these approaches adjust an update according to its age rather than how much each parameter was trained.
...
\solutionheading{independent-stopping}{Stop architecture updates independently} After a shared warm-up period, each client can stop updating its architecture when a local test detects growing preference for skip connections. Other clients continue until they meet their own stopping condition~\cite{collaborative_nas_for_pfl_2025}.

Partial participation and random client selection can cause some clients to contribute more often than others~\cite{optimize_fednas_edge_2021,peaches_2024}. Even if the search continues, frequently available clients influence shared weights and architecture rankings more often. The result may therefore poorly represent clients that participate less often.

\noindent\textbf{\crosscuttingsolutionref{resource-data-groups}.} This solution can select participants by both resources and data coverage so that availability alone does not determine whose data enter the search~\cite{network_aware_fed_nas_2025,finch_2024}.
