...
\noindent\textbf{Solution 1.9a: Design the search for partial participation.} The server can require only a random subset of clients to participate in each round and aggregate the search updates returned by that subset. The participating clients may change between rounds, allowing architecture search to continue without requiring every registered client to remain available~\cite{optimize_fednas_edge_2021}.
