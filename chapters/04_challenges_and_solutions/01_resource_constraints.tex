...
NAS is already challenging in a centralised setting, with the most recent wave of interest sparked as a result of several influential papers that showed the viability of NAS on large amounts of compute as well as papers that focused on making NAS more efficient.  % TODO: Back-up with NASbench 101 claim

Therefore the difficulty of using NAS is amplified in FL, where clients typically feature even less resources. This is one of the key areas which makes NAS-in-FL challenges and has spawned a plethora of challenges and solutions in the literature. We have discovered two main sub-themes of challenges related to client resource constraints: challenges stemming from the limited computational resources available to clients and challenges relating the limited networking capabilities available to clients.

Cross-silo FL suffers less from challenges in this category, since ... . 
...
- These NAS archetypes are typically very demanding of compute and memory and require extra care to make them work on constrained edge devices
- For these NAS archetypes it depends on the situation
