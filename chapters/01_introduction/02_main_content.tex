...
To automate the process of engineering neural network architectures for Deep Learning applications~\cite{nas_survey_2019}, researchers invented \textit{Neural Architecture Search} (NAS) and improved it over the past decade. NAS saves manual labour compared to the traditional architecture engineering approach, whereby a team of domain experts and Deep Learning experts engineer a well-suited architecture based on expert knowledge and trial and error. 

% FL explanation and why it is important
Independantly, but in parallel to NAS, researchers developed a distributed machine learning approach called \textit{Federated Learning} (FL)~\cite{fl_seminal_2017}, whereby \textit{clients} train a model in collaboration with a server without sharing their local data. FL enhances the privacy of client's data, because model trainers can not view it and data is not collected at a central location where a single breach could expose the data of all clients.
