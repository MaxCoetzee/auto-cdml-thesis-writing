...
% why NAS for FL is useful: predefined architectures do not work well in FL 
Engineering neural network architectures in FL is as time-consuming and error-prone (if not more so) % TODO: cite time it takes to engineer
...
% practical problem
The variety in FL settings is large 
Unclear how to design FedNAS method.

Centralised NAS has received significantly more research attention and therefore a large part of FedNAS is just about taking centralised NAS methods and making them work for the FL setting.

The development of new FedNAS methods is hard. 
who struggles: FedNAS developers
what do they struggle with: decdiding 

since the literature is fragmented and 

time-consuming and error-prone process. FL environments
- literature is fragmented
- creating NAS methods is hard in the centralised setting, even more so in the less stable FL setting
- therefore the approach thus far has been to take what works in the centralised setting and adapt it to FL, but this poses some problems

% why use existing centralised NAS methods instead of developing own NAS methods for FL?

% overall problem in FedNAS domain: designing FedNAS methods is hard, make use of centralised NAS as much as possible

% problem with using centralised NAS methods: assumption discrepancies create challenges for using NAS in FL
% why is it still worth using centralised NAS methods if they need adapting, instead developing own FedNAS methods?

Despite the potential benefits of using NAS in FL, it is non-trivial to do so. Since NAS research has focused on a centralised setting, most NAS methods developed thus far are infeasible for direct application in FL, because they make several assumptions about the search process that do not hold in the FL setting. These assumption discrepancies result in \textit{challenges} for using centralised NAS methods in FL, and developers have created \textit{adaptation techniques} for overcoming them. The majority of FedNAS methods developed so far use centralised NAS methods with the help of adaptation techniques.
...
% developing FedNAS methods takes a long time, lots of compute and is error-prone => researchers want to re-use prior work as much as possible 

% practical problem: creating FedNAS methods is hard, rely on prior work, but that is hard, because no literature overview or guide
Developing NAS methods is a time-consuming, compute-hungry affair in a centralised  % TODO: incorporate "800 GPUs for weeks" from https://www.automl.org/nas-bench-101-towards-reproducible-neural-architecture-search/

, error-prone process in a centralised environment and even more so in FL, so researchers try to make use of existing work as much as possible. 
