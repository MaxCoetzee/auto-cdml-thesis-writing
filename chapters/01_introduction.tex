...
Both \textit{Neural Architecture Search} (NAS) and \textit{Federated Learning} (FL) have made significant progress independently in the past decade, and both are widely used in practice. To benefit from the advantages of NAS methods in FL, researchers have started combining them by using NAS in the FL setting~\cite{fednas_2021}~\cite{rt_fed_evo_nas_2020}~\cite{spider_2021}~\cite{fedoras_2022}~\cite{finch_2024}~\cite{efnas_2024}. 
...
% TODO: Move this sentence somewhere and reduce its length: 
% Two major types of FL settings are presented in the literature: the \textit{cross-device} setting, wherein clients are edge devices, and the \textit{cross-silo} setting wherein clients are entire organisations.

% ## why NAS for FL is important: predefined architectures don't work well in FL 
By using NAS in the FL setting, practitioners can potentially avoid the problems faced by pre-defined architectures in the FL setting. In FL, client data is not visible to model developers. This may cause developers to select predefined architectures which contain components redundant for generalising well from some of the client data sets. Additionally, client data tends to be non-i.i.d. Predefined architectures tuned on i.i.d. data potentially underfit on some clients while overfitting on others. Some NAS methods can be used to mitigate these problems by tailoring architectures' capacity to clients' data [TODO: cite].
...
\vspace{1em}
\textbf{Which adaptation techniques are described in the literature, and how do they address FedNAS challenges?}
\vspace{1em}
