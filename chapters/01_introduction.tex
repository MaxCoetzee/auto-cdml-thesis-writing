...
% overview [TODO: may be redundant now]
...
Adapting NAS methods to the FL setting poses a set of challenges similar to the challenges faced by researchers making use of FL in general. We shall make use of a prior work~\cite{fl_taxonomy_2024} that organizes these challenges into classes of similar challenges: % TODO: Should I inlclude why we can use the  taxonomy for our challenges  eventhough it's original purpose is different?
\begin{itemize}
	...
	\item Optimization
		\begin{itemize}
			\item Communication Efficiency: dealing with the high network latency and low bandwith of edge devices
			\item Computation Efficiency: hard to train ML models on low end edge devices
			\item Client selection: clients contribute different amounts of information towards training the shared model, it's hard to select the right clients for a communication round and select clients that will be available for the next communication round
		\end{itemize}
	\item Robustness
		\begin{itemize}
			\item Security
			\item Privacy
			\item Fault-Tolerance: clients are not always available and stragglers need to be dealt with
		\end{itemize}
\end{itemize}
% reforemulate research directions as challenges instead? => not sure

% the specific problem of the domain is the  fragmented literature,e of adaptation techniques, not the FedNAS problem
...
- which FedNAS method surveys exist? -> 2021 survey, multi-objective methods survey, not a single grand overview
- there is not even an overview of FedNAS methods, the last kind-of survey was 2021
...
New NAS methods are constantly developed as the field of NAS research moves forward. When researchers adapt these to the FL setting, they hope to make use of existing literature on adaptation techniques for overcoming the challenge classes relevant to their use case. Unfortunately, no consolidated body of knowledge exists that can inform researchers on existing adaptation techniques. A considerable amount of literature on FedNAS methods has appeared in recent years, resulting in a large number of novel adaptation techniques, but they are scattered throughout the literature. 
This leads us to our research question: Which adaptation techniques are described in the literature and which challenge classes do they overcome in what way?
