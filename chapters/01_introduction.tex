...
Typically, applying Deep Learning to a specific domain involves a process of engineering the architecture of a neural network such that it is well-suited to the application domain. This is done by a team of domain experts and Deep Learning experts. In contrast, Neural Architecture Search (NAS) aims to automate the architecture engineering process \cite{nas_survey_2019}, greatly reducing the manual effort involved. Additionally, NAS can be used to find architectures that perform better than architectures humans have designed for specific application domains~\cite{nasnet_2018}~\cite{amoebanet_2019}~\cite{mobilenetv3_2019}~\cite{efficientnetv2_2021}.

%% FL explanation and why it is important
Federated Learning (FL) is a machine learning method whereby clients collaboratively train a model without sharing their data. enhancing the privacy of client's data.
...
% Meta: I make use of numbering i-iii, because the descriptions of the assumptions are longer phrases and I feel this helps the reader navigate them. If the stated assumptions were only separated by commas, the reader might get lost in the length of the sentence.
